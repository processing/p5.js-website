...
![An illustration showing a 2D coordinate system on the left, showing an origin of (0,0) at its top left, and a 3D coordinate system on the right, showing an origin of (0,0,0) in its center](/images/tutorials/webgl/2d3d_coordinates.png)
...
p5.js has a few functions that we can use to position and orient objects within 3D space: `translate()`, `rotate()`, and `scale()`. Collectively these are known as the _transformation_ of an object. These methods are available for both 2D and 3D drawing.
...
<SketchEmbed
  code={await fs.readFile(
    new URL(
      "./sketches/coordinates-and-transformation/translate/sketch.js",
      import.meta.url,
    ),
  )}
  cssCode={await fs.readFile(
    new URL(
      "./sketches/coordinates-and-transformation/style.css",
      import.meta.url,
    ),
  )}
  bodyCode={await fs.readFile(
    new URL(
      "./sketches/coordinates-and-transformation/translate/index.html",
      import.meta.url,
    ),
  )}
/>
...
<SketchEmbed
  code={await fs.readFile(
    new URL(
      "./sketches/coordinates-and-transformation/rotate/sketch.js",
      import.meta.url,
    ),
  )}
  cssCode={await fs.readFile(
  ...
  bodyCode={await fs.readFile(
    new URL(
      "./sketches/coordinates-and-transformation/rotate/index.html",
      import.meta.url,
    ),
  )}
/>
...
<EditableSketch client:load code={`function setup() {
...
<SketchEmbed
  code={await fs.readFile(
    new URL(
      "./sketches/coordinates-and-transformation/scale/sketch.js",
      import.meta.url,
    ),
  )}
  cssCode={await fs.readFile(
  ...
  bodyCode={await fs.readFile(
    new URL(
      "./sketches/coordinates-and-transformation/scale/index.html",
      import.meta.url,
    ),
  )}
/>
...
_While this is a more advanced topic, each of these transformations affects what is called the model matrix. The model matrix is combined with the view matrix and the projection matrix, both of which help simulate the view of a camera, and this combination results in our 3D scene! You can_ [_learn more about Model View Projection on MDN._](https://developer.mozilla.org/en-US/docs/Web/API/WebGL_API/WebGL_model_view_projection)
...
  let angle = sin(millis() _ 0.005) _ PI \* 0.2;
...
The combined scale, rotation, and translation of a geometry. The verb _transform_ is used when describing altering these properties.
