...
When you want to draw something smooth and curvy instead of straight lines, `splines` are very useful. You might need them to make flowing shapes in generative art, to move a dot or character along a nice path, to draw clean rounded strokes, or to make a loop that looks natural. Splines help because you can place a few points, and p5.js creates a smooth curve through them. Later in this tutorial, you will also learn how to pick exact positions on that curve and find the direction the curve is going, which is helpful for animation and for building shapes that follow the curve.

Splines are smooth curves that interpolate through a series of points, forming gentle, flowing shapes. These curves are defined by a set of points and a function that determines how additional points are computed between them. This makes it easy to draw natural-looking curves.

The sketch below uses the exact same five points on both sides. Only the function that fills in the space between them changes:
...
function setup() {
  ...
  // The same five points are used on both sides.
  let pts = [[20, 130], [55, 50], [90, 140], [125, 40], [160, 110]];

  // Left: vertex() joins the points with straight lines.
  push();
  translate(10, 25);
  noFill();
  stroke(0);
  ...
  noStroke();
  fill(0);
  text('vertex(): straight lines', 10, 175);
  pop();

  // Right: splineVertex() joins the same points with a smooth curve.
  push();
  translate(210, 25);
  noFill();
  stroke(0);
  ...
  noStroke();
  fill(0);
  text('splineVertex(): smooth curve', 10, 175);
  pop();

  describe('Two panels on a light gray background. Each panel shows the same five red points in a zigzag arrangement. In the left panel the points are joined by straight black line segments, labeled "vertex(): straight lines". In the right panel the same points are joined by one smooth black curve, labeled "splineVertex(): smooth curve".');
}
...
Both drawings pass through the identical red points. `vertex()` computes the in-between positions with a straight-line function, so you get sharp corners. `splineVertex()` computes them with a Catmull-Rom function, so the path arrives at and leaves each point in the same direction, which is what makes it look smooth.

<Callout title="Tip">
If you would like a deeper, visual explanation of what makes a curve a spline before you start coding, watch [The Continuity of Splines](https://www.youtube.com/watch?v=jvPPXbo87ds) by Freya Holmér. The opening few minutes give a great overview of why splines exist and how the different families of curves relate to each other. You do not need it to follow this tutorial, but it is a helpful companion.
</Callout>
...
![A blue S-shaped spline curve on a light gray background. Four small red circles mark the control points, and the curve passes exactly through every one of them. A caption below reads "spline() — goes through all 4 points".](../images/2.0/spline-through-points.png)

![A red curve on a light gray background that starts at a red circle in the lower left and ends at a red circle in the upper right. Two more red circles sit away from the curve and are joined to the endpoints by thin gray handle lines, pulling the curve without touching it. A caption below reads "bezier() — only touches the end points".](../images/2.0/bezier-control-handles.png)
...
![Spline curve with tightness = -5 passing through four points.](../images/drawing/tutorial-spline-minus-5.png)
...
![Spline curve with tightness = 5 passing through four points.](../images/drawing/tutorial-spline-plus-5.png)
