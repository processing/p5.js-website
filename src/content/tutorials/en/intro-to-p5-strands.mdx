...
Before p5.js 2.0, you could use [GLSL](https://beta.p5js.org/tutorials/intro-to-glsl/) to write shaders. Shaders run in parallel on the GPU to create visual effects. The GPU can run many similar operations in parallel, much more quickly than the CPU. When you write a p5.js sketch, you are giving the CPU instructions in a sequence. When you add a shader - using p5.strands or GLSL - you are giving instructions to the GPU to run many times at once, simultanously.
...
- **Fragment shaders:** Determine the color of each pixel (_how_ to draw something)
