...
export const completeSketchCode = await readFile(
  "src/content/tutorials/sketches/simple-melody-tutorial/sketch.js",
  "utf-8"
);

export const sketchMiddleC = await readFile(
  "src/content/tutorials/sketches/simple-melody-tutorial/sketch-playMiddleC.js",
  "utf-8"
);

export const sketchStep7 = await readFile(
  "src/content/tutorials/sketches/simple-melody-tutorial/sketch-step7.js",
  "utf-8"
);
...
This tutorial will walk you through the development of a [simple melody app](https://editor.p5js.org/thomasjohnmartinez/sketches/xbGL4GgmS) where users can compose melodies from a musical scale, and play them back.  
...
**Note:** In the next section, [Science of sound and music](#science-of-sound--music), the following concepts are introduced:
...
Skip ahead if you are already familiar with the background concepts above, jump over to the [Building a melody app](#build-simple-app) section to begin your project! 
...
```
Fo {node: Po, ctx: AudioContext, input: GainNode, output: GainNode, frequency: 262…}
```
</Callout>

You can confirm that you created an oscillator object with the correct frequency by examining the “`frequency`” property in the console, or printing it using `console.log(osc.frequency).` The frequency in the `osc.frequency` should match the frequency you identified for your note. 
...
If you do not hear any sounds after clicking the canvas , check that your code looks like this.
<EditableSketch scripts={["https://cdn.jsdelivr.net/npm/p5.sound@latest/dist/p5.sound.min.js"]} code={sketchMiddleC}/>
If a sound does not play, check that your browser settings allow for playing multimedia. You can use the following resources to help you:
...
In the code above, a new `p5.Oscillator` object is initialized with a frequency of 262 Hz and stored in the `osc` variable. The user can start and stop the oscillator by clicking the canvas. To add this interactivity, you added a conditional statement in `mousePressed()` that used the oscillator's `.started` property to check if the oscillator has started playing already. `osc.started` is `true` if the oscillator has started, and `false` otherwise. If the oscillator has started, the conditional instructs the program to stop the oscillator using the `.stop()` function, otherwise the oscillator starts using the `.start()` function.
...
<Callout>
- Display the oscillator's frequency in Hz on the canvas. ([example](https://editor.p5js.org/thomasjohnmartinez/sketches/mVwNkwIh-))
- Play different frequencies by looking at this [chart](https://mixbutton.com/mixing-articles/music-note-to-frequency-chart/) and changing the value of the frequency variable. Round the frequencies to the nearest whole number. ([example](https://editor.p5js.org/thomasjohnmartinez/sketches/_1x5W33c-))
</Callout>
...
Your code should look like this.
<EditableSketch scripts={["https://cdn.jsdelivr.net/npm/p5.sound@latest/dist/p5.sound.min.js"]} code={sketchStep7}/>
...
- Look at the p5.sound.js reference pages and examples found here: [https://p5js.org/examples/](https://p5js.org/examples/).
