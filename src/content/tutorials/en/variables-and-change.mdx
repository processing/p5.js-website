...
  /* places the x and y position of the mouse
...
  //displays the x and y position of the mouse
  on the canvas
  fill(255) //white text
...
For more information, visit the following p5.js reference pages: [f`rameRate()`](/reference/p5/frameRate)`  |  `[`frameCount`](/reference/p5/frameCount)`  |  `[`console.log()`](/reference/console/log)


# Step 5: Add more moving clouds & change frame rate

- Slow down the animation by setting the frame rate to 10:
  - Add this in the line under [`background()`](/reference/p5/background): `frameRate(15); //set frame rate to 15`
- Repeat [Step 4](https://docs.google.com/document/d/14aXzLhuSJv9RXVrayPgv_EVPTsTVbxl7n1Fo3vXXabM/edit#heading=h.8t2tbcn2hswh) to add more clouds to your landscape with different x and y values. 
...
- Subtract the top angle of a triangle that draws leaves (240) by `frameCount % 290 `– the value stored in `cloudOneX`:
  - Modify the value for y<sub>2</sub> in the triangle above by subtracting it by [`frameCount`](/reference/p5/framecount)` % 290`:
...
- modified the value for the y-coordinate of the top corner of the triangle (`y2`) by subtracting it by [`frameCount`](/reference/p5/framecount)` % 290 (`see [triangle()](/reference/p5/triangle) Diagram for a reminder of `y2`)`:`
  ...
  - To achieve this motion, you modified the value for `y2` by subtracting `240 - `[`frameCount`](/reference/p5/framecount)` % 290`. This decreases `y2` by the remainder when [`frameCount`](/reference/p5/framecount) is divided by 290. 
  - As you saw in [Step 5](https://docs.google.com/document/d/14aXzLhuSJv9RXVrayPgv_EVPTsTVbxl7n1Fo3vXXabM/edit#heading=h.s78uuj5sxk1z), each time the [`draw()`](/reference/p5/draw) function runs, the value returned by [`frameCount`](/reference/p5/framecount)`  % 290  `increases by `1` so that `y2 `also changes.
    - Before [`draw()`](/reference/p5/draw) runs, `y2 `is `240`.
    - The first time [`draw()`](/reference/p5/draw) runs, the value from [`frameCount`](/reference/p5/framecount)` % 290` is `1`, and `y2 `becomes `239`.
    - The second time [`draw()`](/reference/p5/draw) runs, the value from [`frameCount`](/reference/p5/framecount)`  % 290  `is` 2`, and `y2 `becomes `238`.
    - The third time [`draw()`](/reference/p5/draw) runs, the value in [`frameCount`](/reference/p5/framecount)`  % 290  `is` 3`, and `y2 `becomes `237`.
    - Once [`frameCount`](/reference/p5/framecount) becomes a number that is a multiple of `290`,` `the value in [`frameCount`](/reference/p5/framecount)`  % 290  `is` 0`, and `y2 `is set back to its original value of `240`.
    - This pattern continues until [`draw()`](/reference/p5/draw) is stopped.
...
[`line()`](/reference/p5/line) Diagram

![A diagram of a line with endpoints (x1,y1) and (x2, y2). The syntax for the line function is displayed above the diagram as: “line(x1, y1, x2, y2);”.](https://lh7-us.googleusercontent.com/9utO1mNtYYbBmLIp5ETv2RsrILKvy7OKoeQwFdQdc2BehfL2a18JmJklw6fxsXQ3AwvZhfTIJpb6awck0oYQAyLJKYcW6R_LfbgIyApqNUW78Lp81-3Wj6p3H4F7WOnhsYl4Kc9JgKwkGoFxA3LjBE4)

[Link to image](https://drive.google.com/file/d/1id4Mu0NrufxwdWvfZFtkCcMxtZw3JZxN/view?usp=sharing)

[Link to p5.js sketch](https://editor.p5js.org/p5Master718/sketches/aVbny4TU0)

Alt text: A diagram of a line with endpoints (x1,y1) and (x2, y2). The syntax for the line function is displayed above the diagram as: “line(x1, y1, x2, y2);”.
...
p5.js uses the console to communicate with programmers about lines of code that it does not understand. These are called [error messages.](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Errors) Programmers use this to locate and fix “*bugs*” in their code. Read the [Field Guide to Debugging](https://docs.google.com/document/d/1Uc5Uey4_UDgix6-O0ez-pcRD-FZQSOaRD9TAGZYzD6c/edit?usp=sharing) or watch [this video](https://www.youtube.com/watch?v=LuGsp5KeJMM\&list=PLRqwX-V7Uu6Zy51Q-x9tMWIv9cueOFTFA\&index=6) to learn more!


# Next Step

- [Conditions and Interactivity](https://docs.google.com/document/d/1n5y27rdwp6hvZ296WZ5WiorQOcfd0aVW6exb8RIm4Io/edit#heading=h.57fjv6kgd059)


# Previous Steps

- [Get Started](https://docs.google.com/document/u/0/d/1q5x5QiPm8N6yRdjks1B-8KQBCzK9vTWQtEvxKiP4o_c/edit)
- [Setting Up Your Environment](https://docs.google.com/document/u/0/d/12AAGiuV1hS2BGacyQdFX6hH4FjrhbVGG7oJ7XYSQHbE/edit)
...
* [p5.js reference pages](https://p5js.org/reference/)
  - [mouseX](/reference/p5/mouseX)
  - [mouseY](/reference/p5/mouseY)
  - [width](/reference/p5/width)
  - [height](/reference/p5/height)
  - [string](/reference/p5/string)
  - [number](/reference/p5/number)
  - [let](/reference/p5/let)
  - [random()](/reference/p5/random)
  - [line()](/reference/p5/line)
  - [triangle()](/reference/p5/triangle)
  - [frameCount](/reference/p5/frameCount)
  - [frameRate()](/reference/p5/frameRate)
