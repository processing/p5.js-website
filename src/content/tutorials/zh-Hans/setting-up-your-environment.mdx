...
开始使用 [p5.js](/reference/) 编写代码之前，您需要设置一个编程环境，以便编写和保存程序。在本指南中，您将学习：

1. 使用在线的 [p5.js Web 编辑器] (https://editor.p5js.org/)，或者在电脑上使用 [Visual Studio Code (VS Code).](https://code.visualstudio.com/) 设置您的编程环境。 
...
您可以选择使用 [p5.js Web 编辑器](https://editor.p5js.org/) 或者 [VS Code](https://code.visualstudio.com/) 开始您的 [p5.js](/reference/) 编程之旅。 
...
[VS Code](https://code.visualstudio.com/) 是一款需要安装在电脑上的流行代码编辑器，它能提供更高级的编码体验。请根据您的实际需求，选择最适合您的编程环境。
...
![指向 p5.js 编辑器右上角“注册”链接的箭头。](../images/introduction/p5_editor_sign_up.png)
...
![指向注册页面中高亮字段的箭头，如上所述](../images/introduction/p5_editor_sign_up_page.png)
...
[p5.js Web 编辑器](https://editor.p5js.org/) 是一个专门供程序员编写和测试 p5.js 代码的在线编程环境。一起来探索 [p5.js Web 编辑器](https://editor.p5js.org/) 的各个组成部分及其功能。   
...
#### 方法一 —— 通过命令面板：
- 点击顶部工具栏中的 *查看*，选择 *命令面板*，或按快捷键 `Ctrl+Shift+P`（Mac 上为 `Cmd+Shift+P`）
- 在搜索栏中输入 *创建新的 p5.js 2.x 项目*，然后选择您想要创建项目的文件夹
- 按照提示配置您的项目
- **当提示时，点击“打开项目”** 在当前 VS Code 窗口中打开新项目；如果您希望保留当前窗口，可以选择 **“在新窗口中打开”** 
- **初学者建议选择“打开项目”**

#### 方法二 —— 通过资源管理器：
- 在资源管理器中右键点击新建文件夹
- 选择“在此处创建 p5.js 2.x 项目”
- 按照提示配置您的项目
-**当提示时，点击“打开项目”** 在当前 VS Code 窗口中打开新项目；如果您希望保留当前窗口，可以选择 **“在新窗口中打开”** 
- **初学者建议选择“打开项目”**
...
上述代码会在预览窗口中创建一个宽 800 像素、高 600 像素的画布元素，并将其背景色设置为一种灰色。
...
- 通过更改 [`background()`](/reference/p5/background) 函数的 *参数* 来改变画布的背景颜色。
  - 将 `background(220);` 修改为 `background("yellow");`，然后点击 *运行*。
...
上面的草图通过在 [`draw()`](/reference/p5/draw) 中调用 [`circle()`](/reference/p5/circle) 函数，在画布上绘制了一个圆形。前两个 *参数* —— `400, 400` —— 将圆心定位在画布的中心，最后一个 *参数* —— `100` —— 表示圆的直径为 100 像素。草图中位于 [`circle()`](/reference/p5/circle) 函数上方的注释行，解释了代码的作用。
...
function draw() {
  ...
  // 在鼠标位置绘制一个直径为 100 的白色圆圈，如步骤 6 所示
  circle(mouseX, mouseY, 100);
}
```
...
编写代码时，很容易拼错函数名或漏掉逗号。语法规则有助于计算机正确解读代码。当某条“规则”被违反时（例如把 [`circle()`](/reference/p5/circle) 拼写错了），浏览器的控制台中就会显示一条消息。这些错误通常被称为“程序错误”；如果你的代码未能正确执行，代码中很可能存在漏洞！
