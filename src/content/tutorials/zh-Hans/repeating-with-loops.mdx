...
![三只粉色的毛毛虫正在画布右侧爬向绿色的终点线。](../images/introduction/caterpillar-race-end.gif)
...
- **条件:** 用于保持循环继续运行的条件。只要该条件为 `true` ，loop 循环就会持续执行。当该条件为 `false`，for 循环就会停止。
...
- 完成毛毛虫的绘制后，将代码封装到名为 `drawCaterpillar()` 的自定义函数中。你可以随意修改毛毛虫的图形！
...
- 在 `draw()` 函数外创建一个新的自定义函数，并将其命名为 `drawCaterpillars()`
  - 在 `drawCaterpillars()` 函数内创建一个新的 for 循环。
    ...
    - 在循环的第二个表达式中，将条件设置为 `i < numCaterpillars` 。
    - 在循环的第三个表达式中递增循环变量的值。
...
- 转到 `drawCaterpillars()` 函数的声明。
  ...
  - 初始化变量 `y` 并将其赋值为： `let y = (i + 0.5) * padding`，其中 `y` 决定了每条毛毛虫的 y 坐标。
...
总而言之，我们已经将 `x` 和 `y` 作为 `drawCaterpillar()` 函数内部使用的参数。在 `drawCaterpillars()` 函数中使用此函数时，我们使用`circX` 和 `y` 作为参数。 
...
- 然后我们更新 `caterpillarEnds` 数组中第 `i` 个位置的值， `drawCaterpillar()` 函数根据这个值来确定毛毛虫的显示位置。
...
- 转到 `drawCaterpillars()` 函数的定义。
  ...
  - 在这行代码上方，通过输入 `let crawl = round(random(3, 6));` 为 `crawl` 变量赋一个新的随机值。
    - 这意味着在程序运行期间，程序会持续绘制长度随机（3 到 6 个圆）的毛毛虫，并将它们从起点移动到终点。
    - 你可以根据希望毛毛虫爬行的长度来修改 `random()` 的值。
