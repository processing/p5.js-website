...
अपने प्रोग्राम के flow को नियंत्रित करना कंप्यूटिंग की शक्ति का एक केंद्रीय हिस्सा है और यही p5.js में रोमांचक एनीमेशन और गेम्स बनाना संभव बनाता है। आमतौर पर, कोड की लाइनों का निष्पादन क्रम में होता है। पिछले ट्यूटोरियल्स में, आपने देखा है कि [`draw()`](/reference/p5/draw) कार्य ऊपर से नीचे तक कोड को बार-बार चलाता है। इससे overlapping shapes और “flip book” जैसी एनीमेशन बन पाती हैं।
...
- [`mouseX`](/reference/p5/mouseX) और [`mouseY`](/reference/p5/mouseY) जैसे built-in चर का उपयोग करना  
- `let` और [assignment operator](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Assignment) का उपयोग करके custom चर को declare, initialize, उपयोग और अपडेट करना  
...
- कोड में टिप्पणी लिखना और गलती messages को समझना — गलती messages और डीबग के बारे में अधिक जानने के लिए, [Field Guide to Debugging](/tutorials/field-guide-to-debugging) पढ़ें।
...
- सूरज के y-निर्देशांक के लिए `sunHeight` नाम का एक custom चर declare करें, और horizon के y-निर्देशांक के लिए `horizon` नाम का एक custom चर बनाएं। `horizon` चर को 200 से initialize करें।  
...
- `sunHeight` को माउस पॉइंटर के y-निर्देशांक, यानी `mouseY`, के बराबर अपडेट करें।
...
- एक सूरज बनाएं जो अपने y-निर्देशांक के लिए custom चर `sunHeight` का उपयोग करता हो।
...
आकृतियों और चर के बारे में अधिक जानने के लिए p5.js reference के [2D shapes](/reference#Shape), [color](/reference#Color), [foundations](/reference#Foundation) और [mouse events](/reference#Mouse) पेज देखें। सामान्य bugs के समाधान के लिए [Field Guide to Debugging](/tutorials/field-guide-to-debugging) (Examples 1 & 2) पढ़ें।
...
[*Comparison ऑपरेटर*](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators) के symbols, जिनका उपयोग आप बूलियन expressions बनाने के लिए कर सकते हैं, निम्नलिखित हैं:
...
if statements और बूलियन values के बारे में अधिक जानने के लिए p5.js reference के [foundations](/reference#Foundation) पेज पर जाएँ।
...
उदाहरण के लिए, आप अपने sunset animation में if statement को संशोधित कर सकते हैं। इसके लिए [`background()`](/reference/p5/background) कार्य को हटाएँ और नीचे दिया गया if-else statement जोड़ें:
...
- सूरज के बाद लैंडस्केप पर एक हरा rectangle बनाएं, ताकि जब सूरज horizon के नीचे हो तो वह छिप जाए। इस rectangle के y-निर्देशांक के लिए `horizon` लाइन की स्थिति का उपयोग करें।  
...
इस स्टेप में आपने अपनी एनीमेशन के लैंडस्केप को customize किया है, जहाँ आपने `background()` और `circle()` कार्य में custom चर का उपयोग किया। ध्यान रखें, सूरज पहाड़ों के पीछे छिपा हुआ है क्योंकि उसे पहले draw किया गया था!
...
- `sunHeight` चर को तब तक बदलते हुए सूरज को चलाएँ जब तक उसकी value 130 से कम हो।  
...
- `sunHeight` चर की value 480 से कम होने तक `redVal` और `greenVal` चर की values बदलकर आसमान के रंग में धीरे-धीरे बदलाव करें।  
...
<Callout>
समय के साथ `sunHeight` की value कैसे बदलती है, यह देखने के लिए `console.log(sunHeight)` का उपयोग करके उसकी value को कंसोल में प्रिंट करें।
...
जब [`draw()`](/reference/p5/draw) पहली बार चलता है, तो background काला होता है और `sunHeight` की value घटती है। जब सूरज का y-निर्देशांक 480 तक पहुँचता है, तब `redVal` और `greenVal` की values बढ़ने लगती हैं और आसमान का रंग धीरे-धीरे बदलना शुरू होता है। पहली conditional statement के कारण, जब `sunHeight` 130 तक पहुँच जाता है, तब सूरज की गति रुक जाती है:
...
ध्यान दें कि यदि पहले if स्टेटमेंट का बूलियन expression `false` है, तो दूसरी condition की जाँच ही नहीं होती। इसका मतलब है कि जब तक `sunHeight` 130 और 480 के बीच है, `redVal` और `greenVal` बदलते रहेंगे। जब `sunHeight` 130 या उससे कम हो जाता है, तो सभी चर बदलना बंद कर देते हैं। इस स्थिति में nested conditional का code block चलने के लिए `sunHeight > 130` और `sunHeight < 480` दोनों का `true` होना आवश्यक है। उदाहरण के लिए [finished project](https://editor.p5js.org/gbenedis@gmail.com/sketches/9lz2aqfTO) देखें, जिसमें [`noStroke()`](/reference/p5/noStroke) को `setup()` में उपयोग किया गया है ताकि आकृतियों की outlines हटाई जा सकें।
...
logical ऑपरेटर के बारे में अधिक जानने के लिए MDN reference में [logical AND (`&&`)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Logical_AND) और [logical OR (`||`)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Logical_OR) देखें।

<Callout>
Logical ऑपरेटर के साथ अभ्यास करने के लिए `draw()` के अंदर निम्नलिखित कोड जोड़ें:
...
`mouseIsPressed` की value माउस बटन दबाने पर कंसोल में कैसे बदलती है, यह देखने के लिए [इस उदाहरण](https://editor.p5js.org/Msqcoding/sketches/CnKfDeaBG) को देखें।
...
- [Mouse and keys](https://editor.p5js.org/Msqcoding/sketches/MQtma5kta): देखें कि `mouseIsPressed` और `keyIsPressed` को logical ऑपरेटर के बिना conditional statements में कैसे उपयोग किया जा सकता है।  
- [Clickable button](https://editor.p5js.org/Msqcoding/sketches/gmRjOEyu_): इस उदाहरण में `mouseIsPressed`, `mouseX`, और `mouseY` का उपयोग करके एक clickable बटन बनाया गया है।  
