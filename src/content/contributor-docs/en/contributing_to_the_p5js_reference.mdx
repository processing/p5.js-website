...
This document describes how to work with p5.js version 2.x, whose reference documentation is currently being hosted at [https://beta.p5js.org/reference/](https://beta.p5js.org/reference/).  If you are documenting code for p5.js v1.x, you should consult [this document](https://p5js.org/contribute/contributing_to_the_p5js_reference/), instead, as some of the syntax and processes are different.
...
In abstract, a comment block for a p5 *function* typically looks as follows.  (We've included the tags that will mark each section but the correct syntax will be shown shortly).
...
### <a id="description" />Describing the function
...
*Hosting* of these images is discussed later in [this section on assets](#using-assets).

To *link* to it, you would use an HTML `img` element, with `src` property referring to the image in `assets/`.  Be sure to also include an appropriate `alt` property.
...
### <a id="method" />`@method` - specifying the function name
...
### <a id="param" />`@param` - specifying details of each parameter
...
1. Following the keyword `@param`, the *type* of the parameter is mentioned in curly brackets `{}`. (in this case, `{Number}`)

* This is strongly recommended but can be omitted in certain cases (notably in p5.strands).

2. After the type, the next *word* (in this case, `angle`) is the name of the parameter.
...
If you need to document a parameter which is expected to be a *function*, you can use the type `Function`.  For example:
...
### <a id="return" />`@return` - specifying the return value
...
1. Following the keyword `@return`, the *type* of the return value is stated in curly brackets `{}`.  (e.g. `{Number}` for the `sin()` function)
...
Here's (a simplification of) the documentation of the *instance* method.  Note that the description also mentions the static version.
...
### <a id="property" />The `@property` tag
...
### <a id="readonly" />The `@readonly` tag
...
![Screenshot of the p5.js reference page of the "fill()" function, showing only one code example section.](src/content/contributor-docs/images/reference-screenshot-example-fill-red.png)
...
### <a id="inserted-examples" />Inserting examples *within* the description

It is possible, and often desirable, to include early runnable examples *within* the description section, before the main "Examples" section of the page.  This is particularly useful when a function or class has a lengthy and detailed description section.
...
Early examples like this allow the reader to quickly play with, and get an early understanding of, a p5.js feature *without* having to read or scroll through possibly overwhelming amounts of documentation discussing in-depth details of the feature.
...
### <a id="norender" />Preventing execution of example code with `norender`

If you do not want the reference page to run an example's code (i.e., you only want the *code* to be shown, not its result), follow the `@example` tag with `// META:norender` on the next line:
...
## <a id="using-assets" />Using assets in examples and descriptions

If your example code (or the description section) uses asset files (e.g. images, fonts, sounds), here's how to work with them.

1. Add the asset file(s) in the *p5.js-website* repo's [/public/assets](https://github.com/processing/p5.js-website/tree/2.0/public/assets) folder.
...
Note that hosting in the *p5.js-website repo* is a significant change from p5 v1.x where such assets were stored instead in the *p5.js* repo.
...
## <a id="linking" />Linking to other p5.js features
...
Note that the URL should *not* include a direct link to the website.
...
<a id="linking-to-constants" />**Exception: Linking to constants**
...
#### <a id="module" />`@module` and `@submodule`

At the top of each source code file will be a comment block with a `@module` tag. A module is a top-level grouping of features in the reference pages on the website.  This does *not* necessarily correspond to any specific software `module` concept in the code itself.
...
The *conceptual* ["3D" module](https://beta.p5js.org/reference/#3D) documents code  mostly from `src/webgl/` (there is no `src/3D/`), and has the following submodules: "Camera", "Interaction", "Lights", "Material", "strands", "p5.Camera", "p5.Shader".
...
It is also used when documenting class properties, to ensure the documentation attaches to the correct class. (Currently class properties are documented *outside* their class.)
...
#### <a id="beta" />The @beta tag - marking experimental API features
...
#### <a id="deprecated" />The @deprecated tag
...
2. Clone [the p5.js-website repo](https://github.com/processing/p5.js-website/tree/2.0) locally.
3. Open a terminal in your new p5.js-website repo
4. Check out the branch "2.0"
5. Run `npm install`
6. Run the following command, using the path to **your** local p5.js repo before the `#`, and the name of **your** branch after the `#`:
...
* Don't forget that if you're [using local asset files](#using-assets), they'll need to be in the *website* repo, *not* the main p5.js repo!
...
## <a id="linting-the-docs" />Linting the comments to find errors
...
Alternatively, you might just want to run it to try to find any potential problems *before* you commit.
...
This will produce *lots* of warnings, across many files!  These do not necessarily mean there is a problem: for various reasons, the codebase uses some documentation tags that documentation.js doesn't understand (but tolerates) in order to organize the reference.

However, you can look through for any warnings relating to the file *you* are working on, to see if anything stands out.  (e.g. a misspelled tag, mismatching parameter name, or "Unknown content")
...
| [@description](#description)           | @description text here                  | If omitted, uses first text before *any* tag.      |
...
| [\`\`\`js example](#inserted-examples) | \`\`\`js example<br />//...<br />\`\`\` | Insert early examples *within* the description.    |
...
## <a id="doc-differences-v1-to-v2" />Appendix: Summary of documentation differences between p5.js v1 and v2
