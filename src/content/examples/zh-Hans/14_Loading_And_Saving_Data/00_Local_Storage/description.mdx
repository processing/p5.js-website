...
title: 局部存储
one-line-description: 在设备上保存来自浏览器的数据。
---

浏览器允许网站在访问者的设备上存储数据。这被称为
<a href="https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage" target="_blank">局部存储</a>。
...
这个例子受到了 Daniel Shiffman 为 Processing 用 Java 编写的加载 JSON 数据和加载表格数据例子的启发。它使用一个
<a href="https://p5js.org/reference/#/p5/class" target="_blank">类</a>
来组织气泡的数据。访问者可以添加新的气泡，它们的数据将被保存在局部存储中。如果访问者重新访问绘图，绘图将重新加载相同的气泡。