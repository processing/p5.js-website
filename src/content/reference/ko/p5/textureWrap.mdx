...
<p><a href="/reference/p5/vertex">vertex()</a>를 호출할 때는 <code>vertex(x, y, z, u, v)</code>에서와 같이 5개의 인수를 포함해야 이미지 내 좌표 <code>(u, v)</code>에 있는 픽셀에 좌표 <code>(x, y, z)</code>의 정점을 매핑할 수 있습니다. 예를 들어, 직사각형 이미지의 모서리는 기본적으로 직사각형의 모서리에 매핑됩니다.
