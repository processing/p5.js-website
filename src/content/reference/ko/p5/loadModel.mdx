...
description: >
  ...
  <p><code>loadModel()</code> 은 OBJ 및 STL 파일에서 3D 모델을 불러올 수 있습니다. 
  모델을 불러오면, 

  <a href="/reference/p5/model">model()</a> 함수를 사용하여,

  예를 들어,
  <code>model(shape)</code> 와 같이 모델을 배치할 수 있습니다.</p>

  <p><code>loadModel()</code> 을 호출할 때는 
  모델을 처리하는 데 도움이 되는 선택적 매개변수를 포함하는 

  세 가지 방법이 있습니다.</p>

  <p>첫 번째 매개변수, <code>path</code> 는 항상 파일의 경로를 나타내는 <code>String</code>
  입니다. 
  ...
  <p><code>loadModel()</code> 을 호출하는 첫 번째 방법은 파일 경로 뒤에 세 가지 선택적 매개변수를 포함합니다.
  첫 번째 선택적 매개변수인 
    ...
    params:
      ...
      - name: failureCallback
        description: >
          ...
          <code>Error</code> 객체를 받습니다.</p>
        type: Function(Event)
        optional: true
      - name: fileType
        description: >
          <p>모델의 파일 확장자. <code>'.obj'</code> or
          <code>'.stl'</code>.</p>
        type: String
        optional: true
    return:
