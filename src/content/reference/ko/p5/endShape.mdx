...
description: >
  <p>'endShape()'함수는 사용자 정의 도형에 꼭지점을 추가하기 시작합니다.</p>
  ...
  기본적으로 도형의 첫 번째와 마지막 꼭지점은 연결되지 않습니다. 

  <code>endShape(CLOSE)</code>
  와 같이

  <code>CLOSE</code> 상수를 전달하면
  첫 번째와 마지막 꼭지점이 연결됩니다.</p>

  <p>두 번째 매개변수, <code>count</code> 역시 선택사항입니다.
  WebGL 모드에서, <code>count</code> 는

  는 동일한 도형의 많은 복사본을 효율적으로 그리기 위해 사용되는
  ...
  <code>endShape()</code> 를 호출하면 도형에 꼭지점을 추가하는 것이 중단됩니다.
...
params:
  - name: mode
    description: |
      <p>CLOSE를 사용하여 모양을 닫습니다e</p>
    type: Constant
    optional: true
  - name: count
    description: >
      <p> 여러 복사본을 그리는 데 유용합니다.(WebGL
      모드에서).</p>
    type: Integer
    optional: true
chainable: true
