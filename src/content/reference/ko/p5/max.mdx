---
title: max
module: 수학
submodule: 계산
file: src/math/calculation.js
description: |-
  <p>여러 개의 수 중 가장 큰 값을 반환합니다.</p>
  <p>하나의 매개변수가 있는 <code>max()</code>는 이를 수의 배열로 해석하고 가장 큰 수를 반환합니다.</p>
  <p>두 개 이상의 매개변수가 있는 <code>max()</code>는 이를 개별 수로 해석하고 가장 큰 수를 반환합니다.</p>
line: '618'
isConstructor: 'false'
itemtype: 메서드
example:
  - |-

    <div>
    <code>
    function setup() {
      createCanvas(100, 100);

      background(200);

      // Calculate the maximum of 10, 5, and 20.
      let m = max(10, 5, 20);

      // Style the text.
      textAlign(CENTER);
      textSize(16);

      // Display the max.
      text(m, 50, 50);

      describe('The number 20 written in the middle of a gray square.');
    }
    </code>
    </div>

    <div>
    <code>
    function setup() {
      createCanvas(100, 100);

      background(200);

      // Create an array of numbers.
      let numbers = [10, 5, 20];

      // Calculate the maximum of the array.
      let m = max(numbers);

      // Style the text.
      textAlign(CENTER);
      textSize(16);

      // Display the max.
      text(m, 50, 50);

      describe('The number 20 written in the middle of a gray square.');
    }
    </code>
    </div>
class: p5
return:
  description: 가장 큰 수
  type: Number
overloads:
  - line: '618'
    params:
      - name: n0
        description: |2-

          <p>비교할 첫번째 수</p>
        type: Number
      - name: n1
        description: |2-

          <p>비교할 두번째 수</p>
        type: Number
    return:
      description: 가장 큰 수
      type: Number
  - line: '680'
    params:
      - name: nums
        description: |2-

          <p>비교할 여러 개의 수</p>
        type: Number[]
    return:
      description: ''
      type: Number
chainable: 'false'
---

# max
