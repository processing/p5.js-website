...
description: >
  ...
  <p>첫 번째와 두 번째 파라미터, <code>x</code> 와 <code>y</code> 는 
  꼭짓점의 

  x, y 좌표를 설정합니다.</p>

  <p>세 번째 파라미터, <code>z</code> 는 선택 사항입니다.  
  WebGL 모드에서 꼭짓점의 z 좌표를 설정하며, 

  기본적으로 , <code>z</code> 는 0입니다.</p>

  <p>네 번째와 다섯 번째 파라미터, <code>u</code> and <code>v</code> 와
  역시 선택 사항입니다.

  이것은  <a href="/reference/p5/endShape">endShape()</a>

  함수와 함께 사용될 때 꼭짓점의 텍스처 u, v 좌표를 설정합니다. 기본적으로, <code>u</code>
  와 <code>v</code>는 둘 다 0입니다.</p>
line: 1850
isConstructor: false
itemtype: method
example:
...
overloads:
  ...
  - line: 2028
    params:
      ...
      - name: u
        description: |
          <p>꼭지점 텍스처의 u-좌표. 기본값은 0.</p>
        type: Number
        optional: true
      - name: v
        description: |
          <p>꼭지점 텍스처의 v-좌표. 기본값은 0.</p>
        type: Number
        optional: true
    chainable: 1
chainable: true
