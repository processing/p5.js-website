...
description: >
  ...
  이 곡선은 두 개의 앵커 포인트와 두 개의 제어 포인트로 정의됩니다. 

  베지어 곡선은 <a href="/reference/p5/curve">curve()</a>

  함수로 생성하는 스플라인 곡선보다 더 많은 제어가 가능합니다.</p>

  <p>처음으로, 두개의 매개 변수인 <code>x1</code> 와 <code>y1</code>,
  는 다음을 설정합니다.

  첫 번째 앵커 포인트는 곡선이 시작되는 곳입니다.</p>

  <p>다음으로 네 개의 매개 변수인, <code>x2</code>, <code>y2</code>,
  <code>x3</code>, 그리고 <code>y3</code>, 두 제어 포인트를 설정합니다.

  제어 포인트는 곡선을 그 방향으로 '끌어당깁니다'.</p>

  <p>일곱 번째와 여덟 번째 매개 변수, <code>x4</code> 와 <code>y4</code>, 
  는 마지막 앵커 포인트를 설정합니다. 
  ...
  3D 버전의 <code>bezier()</code> 는 각 점이 x-, y-, z-좌표를 가지고 있기 때문에 
...
overloads:
  - line: 13
    params:
      ...
      - name: x2
        description: |
          <p>첫 번째 제어 포인트의 x 좌표입니다.</p>
        type: Number
      - name: y2
        description: |
          <p>첫 번째 제어 포인트의 y 좌표입니다.</p>
        type: Number
      - name: x3
        description: |
          <p>두 번째 제어 포인트의 x 좌표입니다.</p>
        type: Number
      - name: y3
        description: |
          <p>두 번째 제어 포인트의 y 좌표입니다.</p>
        type: Number
      - name: x4
        description: |
