...
description: >
  ...
  <p>Calling <code>textureWrap()</code> with no arguments returns an object with
  the current

  mode for x and y directions, as in <code>{ wrapX: CLAMP, wrapY: REPEAT
  }</code>.</p>
line: 2996
isConstructor: false
itemtype: method
...
class: p5
return:
  description: The current texture wrapping for x and y.
overloads:
  - params:
      - name: wrapX
