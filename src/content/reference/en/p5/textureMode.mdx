...
description: >
  ...
  endShape();</code></pre><p><code>textureMode()</code> changes the coordinate
  system for uv coordinates.</p>
  ...
  <pre><code class="language-js">// Set the texture mode to use normalized
  coordinates.
  ...
  endShape();</code></pre><p>By default, <code>mode</code> is
  <code>IMAGE</code>, which scales uv coordinates to the
  ...
  <p>Calling <code>textureMode()</code> with no arguments returns the current
  texture mode.</p>
line: 2723
isConstructor: false
itemtype: method
...
class: p5
return:
  description: 'The current texture mode, either IMAGE or NORMAL.'
  type: IMAGE|NORMAL
overloads:
  - params:
      - name: mode
