...
description: >-
  ...
  }</code></pre><p>p5.strands is an experimental mode for writing shader code
  with p5.js-like

  syntax instead of GLSL. In p5.strands shader callbacks, <code>color()</code>
  accepts

  the same input formats but returns a <code>vec4</code> instead of a
  <code>p5.Color</code>

  object, with RGBA components normalized to the 0–1 range. All colors in

  strands are RGB-based; <code>colorMode()</code> has no effect inside shader

  callbacks. Color utility functions such as <code>red()</code>,
  <code>green()</code>, <code>blue()</code>,

  <code>alpha()</code>, <code>hue()</code>, <code>saturation()</code>,
  <code>brightness()</code>, and <code>lightness()</code>

  also return values in the 0–1 range when used in strands.</p>
