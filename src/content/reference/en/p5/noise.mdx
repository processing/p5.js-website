...
description: >-
  ...
  }</code></pre><p><code>noise()</code> can also be used in shaders with
  p5.strands, where it returns

  values in the range 0 to 1. The example below uses <code>noise()</code> inside
  a

  filter shader to create a cloud-like texture effect:</p>
